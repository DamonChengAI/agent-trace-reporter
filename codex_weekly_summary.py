#!/usr/bin/env python3
"""Generate weekly Markdown reports from Codex JSONL logs."""
...
def extract_real_user_request(text):
    ...
    if stripped.startswith("<") and not stripped.startswith("<http"):
        return None
    if stripped.startswith("# AGENTS.md") or stripped.startswith("# CODEX"):
        return None
...
def scan_messages(input_dir):
    ...
    for path in sorted(base.rglob("*.jsonl")):
        ...
        for obj in read_jsonl(path):
            ...
            if obj_type == "user_message":
                text = extract_real_user_request(payload.get("message", ""))
                ...
                if text and timestamp:
                    messages.append(
                        {
                            "timestamp": timestamp,
                            "project": derive_project_name(cwd, path.parent.name),
                            "text": text.strip(),
                        }
                    )
                continue
            ...
            text = extract_real_user_request(raw_text)
            timestamp = parse_timestamp(obj.get("timestamp"))
            if text and timestamp:
                messages.append(
                    {
                        "timestamp": timestamp,
                        "project": derive_project_name(session_cwd, path.parent.name),
                        "text": text.strip(),
                    }
                )
...
def build_parser():
    parser = argparse.ArgumentParser(description="Generate weekly reports from Codex JSONL logs.")
    parser.add_argument("--input-dir", required=True, help="Directory containing Codex JSONL files.")
